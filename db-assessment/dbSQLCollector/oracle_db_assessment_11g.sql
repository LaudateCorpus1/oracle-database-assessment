...
SELECT h.snap_id,
       ...
       h.module,
       rank() over(partition by h.snap_id, h.instance_number, h.con_id order by h.executions_delta desc) rank_exec,
       h.executions_delta exec_delta,
       rank() over(partition by h.snap_id, h.instance_number, h.con_id order by h.elapsed_time_delta desc) rank_elap,
       h.elapsed_time_delta,
       rank() over(partition by h.snap_id, h.instance_number, h.con_id order by ROUND(h.rows_processed_delta / h.executions_delta) desc) rank_rows_exec,
       ROUND(h.rows_processed_delta / h.executions_delta) rows_per_exec,
       rank() over(partition by h.snap_id, h.instance_number, h.con_id order by ROUND(h.rows_processed_delta / h.buffer_gets_delta, 3) desc) rank_rows_buff,
       ROUND(h.rows_processed_delta / h.buffer_gets_delta, 3) rows_per_buffer,
       rank() over(partition by h.snap_id, h.instance_number, h.con_id order by ROUND(h.elapsed_time_delta / h.executions_delta, 3) desc) rank_exec_elap,
...
spool opdb__topsegments__&v_tag
...
spool opdb__ashprograms__&v_tag
...
spool opdb__ashsqlwaits__&v_tag
